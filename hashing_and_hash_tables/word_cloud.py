class WordCloud:
        ...
        for i, character in enumerate(input_string):
            ...
            if i == len(input_string) - 1:
                ...
                if current_word_length > 0:
                    current_word_length = self.slice_word(input_string, current_word_start_index, current_word_length)
    ...
    def add_word_to_dictionary(self, word):
        # If the word is already in the dictionary we increment count
        ...
        # If an uppercase version is in the dictionary, we know our input must be lowercase
        # since we only include uppercase words if they're always uppercase, we add the 
        # lowercase version and give it the uppercase version's count
        elif word.capitalize() in self.words_to_counts:
            self.words_to_counts[word.lower()] = 1
            self.words_to_counts[word.lower()] += self.words_to_counts[word.capitalize()]
    ...
    def slice_word(self, input_string, start_index, word_length):
        current_word = input_string[start_index: start_index + word_length]
        self.add_word_to_dictionary(current_word)

        # Reset word_length
        return 0

words = WordCloud('What do we say to the God of Death? We say....... "not today", what else?')
print(words.words_to_counts)